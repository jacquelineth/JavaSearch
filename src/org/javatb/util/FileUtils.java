...
public final class FileUtils
{
   ...
   */
  public static final Set<String> ZIP_EXT = Collections.unmodifiableSet(SearchEngine.asSet(Configuration.getArrayProperty(ConfigurationProperty.ZIP_EXTENSIONS)));
   ...
   * @throws IOException if an IO error occurs while looking up the file.
   */
  public static Reader getFileReader(final File path) throws IOException
  {
   ...
   */
  public static String getFileShortName(final String filePath)
  {
    int idx = filePath.lastIndexOf('/');
   ...
   */
  public static byte[] zipEntryAsBytes(final ZipInputStream zis) throws Exception
  {
   ...
   */
  public static void openSearchElement(final SearchElement elt)
  {
   ...
   */
  public static void editSearchElement(final SearchElement elt)
  {
    performAction(elt, Desktop.Action.EDIT);
  }

  /**
   * Browse a search element with its OS URI
   * @param elt  the element to browse
   * @author TJ
   */
  public static void exploreOnSearchElement(SearchElement elt)
    {
        if (!Desktop.isDesktopSupported()) return;
        File file;
        //for files, use default mechanism
        //for archive retrieve the containing archive name
        if (elt.isFile())
            file = new File(elt.toString());
        else
            file = new File(elt.getPath().get(0).getName());
   ...
   */
  private static void performAction(final SearchElement elt, final Desktop.Action action)
  {
    try
    {
      if (!Desktop.isDesktopSupported()) return;
   ...
   */
  private static byte[] getEntryAsBytes(final ZipInputStream zis, final String name) throws Exception
  {
    try
    {
  ...
  /**
   * Recursively delete the file hierarchy dnoted by th specified root.
   * @param root the root of the file hierarchy to delete, either a file or a directory.
   */
  public static void deleteRecursive(final File root)
  {
    try
    {
      if (root.isDirectory())
